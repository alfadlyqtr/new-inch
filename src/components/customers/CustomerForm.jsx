...
export default function CustomerForm({ initial, onCancel, onSave }) {
  ...
  function addCustom(){
    ...
    setForm(prev => {
      ...
      const pts = [...(prev.measurements.custom_points||[]), { id, label: 'Custom', xPct: 50, yPct: 50, value: '', unit: 'cm' }]
  ...
  function setCustom(i, k, v){
    setForm(prev => {
      ...
      const it = list[i]
      if (it?.id){
        pts = pts.map(p => p.id === it.id ? { ...p, label: k==='name' ? (v||'Custom') : p.label, value: k==='value'? v : p.value, unit: k==='unit'? v : p.unit } : p)
      }
    ...
    <div className="space-y-4">
      ...
      <div className="space-y-2">
        <div className="flex items-center justify-between gap-2">
          <div className="text-white/80 font-medium">Visual Measurements</div>
          <div className="flex items-center gap-2 text-xs">
            <label className="inline-flex items-center gap-1"><input type="checkbox" checked={addMode} onChange={(e)=> setAddMode(e.target.checked)} /> <span className="text-slate-300">Add label</span></label>
            <label className="inline-flex items-center gap-1"><input type="checkbox" checked={moveFixed} onChange={(e)=> setMoveFixed(e.target.checked)} /> <span className="text-slate-300">Move built-ins</span></label>
            <button type="button" onClick={clearLabels} className="px-2 py-1 rounded bg-white/10 border border-white/10 text-slate-200">Clear custom labels</button>
          </div>
        </div>
        <MeasurementOverlay
      ...
      <div className="space-y-2">
        ...
        <div className="grid gap-3 sm:grid-cols-3">
          {['chest','waist','hips','shoulders','sleeve_length','neck','inseam','outseam','thigh'].map(k => (
            <div key={k}>
              <label className="text-xs text-slate-400 capitalize">{k.replace('_',' ')}</label>
        ...
        <div>
          <div className="flex items-center justify-between mt-2">
            <div className="text-xs text-slate-400">Custom measurements</div>
            <button type="button" onClick={addCustom} className="px-2 py-1 rounded bg-white/10 border border-white/10 text-xs">+ Add</button>
          </div>
          <div className="mt-2 space-y-2">
            {customList.map((m, i) => (
              <div key={i} className="grid grid-cols-6 gap-2">
                <input value={m.name} onChange={(e)=> setCustom(i,'name',e.target.value)} placeholder="Name" className="col-span-2 rounded bg-white/5 border border-white/10 px-2 py-1.5 text-sm text-white" />
                <input value={m.value} onChange={(e)=> setCustom(i,'value',e.target.value)} placeholder="Value" className="col-span-2 rounded bg-white/5 border border-white/10 px-2 py-1.5 text-sm text-white" />
                ...
                <button onClick={()=> rmCustom(i)} className="px-2 py-1 rounded bg-red-500/10 border border-red-500/30 text-red-200 text-xs">Remove</button>
              </div>
            ))}
          </div>
        </div>
      ...
      {/* Preferences */}
      <div className="space-y-2">
        <div className="text-white/80 font-medium">Preferences</div>
        <div className="grid gap-3 sm:grid-cols-3">
          <div>
            <label className="text-xs text-slate-400">Preferred Fabric</label>
          ...
          <div className="sm:col-span-2">
            <label className="text-xs text-slate-400">Style Notes</label>
          ...
          <div className="sm:col-span-3">
            <label className="text-xs text-slate-400">Special Requirements</label>
      ...
      <div className="flex items-center justify-end gap-2 pt-2">
        <button onClick={onCancel} className="px-3 py-1.5 rounded bg-white/5 border border-white/10">Cancel</button>
        <button onClick={submit} disabled={saving} className="px-3 py-1.5 rounded pill-active glow">{saving ? 'Saving…' : (initial ? 'Save Changes' : 'Create Customer')}</button>
      </div>
    </div>
  )
}
