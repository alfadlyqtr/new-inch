...
export default function TimeTrackingSystem() {
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        ...
        const base = { business_id: ua.business_id, staff_id: user.id, staff_name: ua.full_name || user.email || 'Staff' }
        if (!mounted) return
        setIds(base)
        await reload(base, range)
      } finally {
  ...
  async function reload(base = ids, r = range) {
    const data = await AttendanceApi.list({ business_id: base.business_id, staff_id: base.staff_id, start: r.start, end: r.end })
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div className="text-white/90 font-medium">My Attendance</div>
        <div className="flex items-center gap-2 text-sm">
          <input type="date" value={range.start || ''} onChange={(e)=> setRange((p)=>({ ...p, start: e.target.value || null }))} className="rounded bg-white/5 border border-white/10 px-2 py-1 text-white" />
          <span>→</span>
          <input type="date" value={range.end || ''} onChange={(e)=> setRange((p)=>({ ...p, end: e.target.value || null }))} className="rounded bg-white/5 border border-white/10 px-2 py-1 text-white" />
          <button onClick={()=> reload()} className="px-2 py-1 rounded bg-white/10 border border-white/10 text-slate-200">Apply</button>
        </div>
      </div>
      <div className="rounded-xl border border-white/10 bg-white/5 overflow-hidden">
        <table className="w-full text-sm">
          ...
          <tbody>
            {rows.length === 0 && (
              <tr><td colSpan="7" className="p-3 text-slate-400">No records.</td></tr>
            )}
            {rows.map(r => (
